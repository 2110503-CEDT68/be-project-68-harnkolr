    ...
    try{
        ...
        if(!hospital){
            ...
            if(existedAppointments.length >= 3 && req.user.role !== 'admin'){
                return res.status(400).json({success:false,message:`The user with ID ${req.user.body} has already made 3 appointments`});
            }
...
exports.updateAppointment = async (req,res,next)=>{
    try{
        ...
        if(appointment.user.toString() !== req.user.id && req.user.role !== 'admin'){
            return res.status(401).json({success:false , message : `User ${req.user.id} is not author`})
        }
